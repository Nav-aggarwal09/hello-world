...
#What this function does is solve a maze using a technique known as
        #recursive backtracking. This is a very powerful tool that can be
        #used to solve otherwise difficult problems. In this case, the function
        #takes a row and a column that corresponds to a point on the maze and
        #attempts to find a solution from that row and column. If a solution can
        #be found, the function returns true. Otherwise, the function returns
        #false. Along the way, we will keep track of the points we've already
        #visited, but we have to be clever about how we do this. That is where
        #the "backtracking" comes into play. Once this function is filled in
        #correctly, we will be able to run the code in this file and see a
        #complete maze solving application.
